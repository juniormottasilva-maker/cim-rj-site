...
(function () {
  ...
  function itemHTML(m) {
    ...
    return `
      <a href="/municipios/${m.slug}.html" class="cimg-item"
      ...
      timer = setTimeout(async () => {
        ...
        if (resultados.length === 0) {
          ...
          box.innerHTML = termo.trim().length >= 2
            ? `<div style="padding:14px 16px;color:var(--muted);font-size:13px;">Nenhum resultado ainda para "${termo}". A base cobre os municípios já pesquisados — mais chegam a cada dia.</div>`
            : "";
          return;
        }
